...
2019年: RFC (Request for Comments)を公開し、コミュニティからのフィードバックを集約

実際のユースケースから得られた知見:

- 1000行を超える大規模コンポーネントでの改善
- 複雑なフォーム処理の明瞭化
- 共通ロジックの抽出と再利用の簡素化
...
Vue 3では両方のAPIスタイルをサポート:

- 既存プロジェクトはOptions APIのまま
- 新機能にComposition APIを段階的に導入可能
- 同一コンポーネント内で両APIの混在も可能

```jsx
export default {
  data() { return { legacy: true } },
